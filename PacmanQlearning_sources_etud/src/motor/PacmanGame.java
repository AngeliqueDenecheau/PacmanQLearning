...
public class PacmanGame extends Game {
	...
	private static final int TOUR_MAX_INVINCIBLE = 30;
	...
	public void moveAgent(Agent agent, AgentAction action) {
		// si le mouvement est autorisé (pas un mur)
		//if (isLegalMove(agent, action)) {
			...
			if (agent.get_type() == typeAgent.PACMAN) { // si l'agent est pacman
				// si pacman rencontre une capsule
				if (_maze.isCapsule(dirX, dirY))
					eatCapsule(dirX, dirY);
				    this.nbCapsule -= 1;
				    
				if (_maze.isFood(dirX, dirY))
					eatGum(dirX, dirY); // on mange la gum
					this.nbFood -= 1;
...
//			fantom.set_alive(true);
//			fantom.set_position(fantom.get_first_position());
//		}
		if(nightmareMode) {
			_agentsFantom = Factory.giveMeOnlyHuntersFantom(_maze.getGhosts_start());
		} else {
	...
	public void takeTurn() {
		...
		pacman.update(this, pacman.get_position(), a, reward, isFinalState);
	...
	public boolean checkIfFinalState() {
		...
		boolean fin_gum; // fin = vrai , la partie est fini il n'y a plus de nourriture
		boolean fin_pacman; // fin = vrai , la partie est fini, il n'y a plus de pacman
	...
	public Agent get_agentsPacman() {
		return pacman;
	}

	public void set_agentsPacman(Agent _agentsPacman) {
		this.pacman = pacman;
	}
